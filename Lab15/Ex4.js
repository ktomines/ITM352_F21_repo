...
//          FOR ASSIGNMENT 3!!!! 
//shows how many times user visited site
//DIFF COOKIE DATA; SEMI-PERMANENT DATA
//if page views existed then we increment it by 1
...
// session ID that it creates for you 
app.get('/set_session', function (req, res, next) {
    res.send(`welcome, your session ID is ${req.session.id}`);
    next();
});
// retrieves same string
app.get("/use_session", function (request, response) {
    response.send("Your session ID is " + request.session.id);
    request.session.destroy(); //destroys session; makes a diff session id every time when refreshed
}
);


// asks what users fav color is
...
// grabs color & stores it in a session
app.post("/fav_color", function (request, response) {
var POST = request.body;
request.session.fav_color = POST["color"];
response.send("ur fav color is " = request.session.fav_color);
});
